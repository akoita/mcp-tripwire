...
- **Model is `gemini-3-pro-preview` by default, overridable via `TRIPWIRE_AGENT_MODEL`** — single-sourced in [`src/tripwire/agents/_model.py`](../../src/tripwire/agents/_model.py). The original hard-coded `gemini-3-pro` does not exist on the AI Studio `v1beta` endpoint (every playground turn failed `404 NOT_FOUND`), which was the explicit ask the agents-cli "never change models" convention requires.
