...
- **Coarse precision on real manifests (false positives)** — common benign patterns are now handled: JSON-Schema `$schema` / `$id` metadata is excluded from scanned text, and the ordinary English verb "fetch" no longer trips `EXF-URL`. The [Morpho manifest fixture](../../corpus/samples/morpho-tools.json) is an all-tools clean regression. Pattern scanning remains best-effort, and novel benign descriptors may still over-fire.
