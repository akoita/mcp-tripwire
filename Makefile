...
demo-real-mcp:  ## Real upstream demo: Tripwire fronts Microsoft Playwright MCP via npx
	@if command -v uv >/dev/null 2>&1; then PYTHONPATH=src:. uv run python examples/demo_real_mcp_playwright.py; else PYTHONPATH=src:. $(RUN_PYTHON) examples/demo_real_mcp_playwright.py || PYTHONPATH=src:. $(PYTHON) examples/demo_real_mcp_playwright.py; fi
