...
- **Code change ⇒ feature catalog change** (machine-enforced on `git push`). Any change under `src/tripwire/`, `app/`, or `examples/` must touch at least one page under `docs/features/` in the same branch. Enforced by the `features-diff-coverage` pre-push hook ([`scripts/check_features_diff_coverage.sh`](scripts/check_features_diff_coverage.sh)). Escape hatch: include the literal token `[docs-skip]` in any commit message in the branch (for typos, internal refactors with no consumer-visible change, etc.). Graceful no-op when `docs/features/` doesn't exist on this branch.
